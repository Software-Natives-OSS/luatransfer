...
describe('Test TransferLua SendFile', () => {
    ...
    it('Syntax errors are not reported if compiled at target', () => {
        const options = transferLua.combineOptions(transferLua.OPTION_EXECUTE);
        this.transfer.sendFile(getFileLocation('syntax_error.lua'), this.stateName, { options: options });
    });

    it('Should throw in case of syntax error and compiled before donwloaded', () => {
        const options = transferLua.combineOptions(transferLua.OPTION_EXECUTE, transferLua.OPTION_COMPILE_BEFORE);
        assert.throws(() => {
            this.transfer.sendFile(getFileLocation('syntax_error.lua'), this.stateName, { options: options });
        });
    });
...
describe('Test TransferLua SendChunk', () => {
    ...
    it('Should throw if a chunk contains syntax errors and compiled before downloading', () => {
        const options = transferLua.combineOptions(transferLua.OPTION_EXECUTE, transferLua.OPTION_COMPILE_BEFORE);
        assert.throws(() => {
            this.transfer.sendChunk('AScriptName.lua', this.stateName, 'print(Hello syntax error!)', { options: options });
        });
